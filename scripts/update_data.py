...
def fetch_yahoo_history(ticker, period="60d"):
...
def get_aligned_financial_data(days=60):
...
def parse_fuel_xml(xml_bytes, names_map):
    ...
    # To filter out inactive/closed stations, we only keep prices updated in the last 30 days
...
def update_database(stations, finance_data):
    """Updates the static JSON files on a rolling 30-day window basis."""
    ...
    # Sort and keep rolling 60 days of national history (gives a nice chart)
    ...
    cutoff_history = (datetime.date.today() - datetime.timedelta(days=30)).isoformat()
    ...
    for cp in updated_cps:
        ...
        for s in stations_by_cp[cp]:
            ...
            for fuel, p_info in s["prices"].items():
                ...
                # Sort and clean history older than 30 days
